"""enable Postgres row-level security for inventory_cycles, inventory_counts,
...
Own migration, not merged into 0009 — every prior phase has kept schema and RLS separate
(0001/0002, 0004-0006, 0007/0008). Replicates the exact FORCE ROW LEVEL SECURITY + fail-closed
tenant_id policy pattern from 0002/0006/0008.
...
_RLS_TABLES = ["inventory_cycles", "inventory_counts", "reconciliation_actions"]

# Same fail-closed policy as 0002/0006/0008: current_setting(..., true) returns NULL rather
# than raising when the session variable hasn't been set, so a query that runs without a
